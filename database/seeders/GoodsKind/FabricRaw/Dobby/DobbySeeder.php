<?php

namespace Database\Seeders\GoodsKind\FabricRaw\Dobby;

use App\Models\LineProduct\GoodsKind;
use App\Models\LineProduct\Machine\MachineModuleType;
use App\Models\LineProduct\Station;
use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Str;

class DobbySeeder extends Seeder {
    /**
     * Run the database seeds.
     * وضعیت های تولید ماشین در ماژول های دابی و بادامکی شرکت های بزرگ
     * @return array[]
     */
    public function getInfo() {
        return [
            7001 => [
                "caption"         => "کارت تولید",
                "status_type_id"  => 7001,
                "controller_info" => \App\Http\Controllers\GoodsKindProcess\FabricRaw\Dobby\ProductionCard\DashboardController::get_controller_info_for_permission()
            ],
            7002 => [
                "caption"         => "فرم تولید",
                "status_type_id"  => 7002,
                "controller_info" => \App\Http\Controllers\GoodsKindProcess\FabricRaw\Dobby\ProductionForm\DashboardController::get_controller_info_all()
            ],
            7003 => [
                "caption"         => " ماشین ها",
                "status_type_id"  => 7003,
                "controller_info" => \App\Http\Controllers\GoodsKindProcess\FabricRaw\Dobby\Machine\DashboardController::get_controller_info(),
                "code"            => [
                    7003001,
                    7003003,
                    7003004,
                    7003005,
                    7003006,
                    7003009,
                    7003010,
                    7003011,
                    7003012,
                    7003013,
                    7003014,
                    7003015,
                    7003016,
                    7003017,
                    7003018,
                    7003019,
                    7003020,
                    7003021,
                    7003022,
                    7003023,
                    7003024,
                    7003025,
                    7003025,
                    7003026,
                    7003027,
                    7003028,
                    7003029,
                    7003030,
                    7003031,
                    7003032,
                    7003033,
                    7003034,
                    7003035,
                    7003036,
                    7003037,
                    7003038,
                    7003039,
                    7003040,
                    7003041,
                ]
            ],
            7004 => [
                "caption"         => "فرم طراحی",
                "status_type_id"  => 7004,
                "controller_info" => \App\Http\Controllers\GoodsKindProcess\FabricRaw\Dobby\DesignForm\DashboardController::get_controller_info()
            ],
            7006 => [
                "caption"         => "پارچه های در انتظار بسته بندی",
                "status_type_id"  => 7006,
                "controller_info" => \App\Http\Controllers\GoodsKindProcess\FabricRaw\Dobby\Packing\FabricWaitingForPackingController::get_controller_info()
            ],
            7007 => [
                "caption"         => "بسته بندی پارچه خام",
                "status_type_id"  => 7007,
                "controller_info" => \App\Http\Controllers\GoodsKindProcess\FabricRaw\Dobby\Packing\DashboardController::get_controller_info()
            ],

        ];
    }

    public function getFullId() {
// این شناسه فقط برای ماشین استفاده می شود.
        return "7003001";

    }

    public $machine_module_code = "dobby";


    public function run() {


        $machine_module_type_id = MachineModuleType::where( "code", $this->machine_module_code )->first()->id;

        // Button List

        $info = $this->getInfo();

        DB::table( "buttons" )->where( [ "status_type_id" => $this->getFullId() ] )->delete();

        foreach ( $info[7003]["controller_info"] as $key => $item ) {
            $id      = $this->getFullId() . $key;
            $name    = $item["route"] . "index";
            $caption = $item["button"]["caption"];

            DB::table( "buttons" )->insert( [
                "id"             => $id,
                "name"           => $name,
                "caption"        => $caption,
                "status_type_id" => $this->getFullId()
            ] );

        }


        DB::table( "machine_status" )->where(
            [
                "machine_module_type_id" => $machine_module_type_id
            ] )->whereNotIn("production_status_id", $info[7003]["code"])->delete();

        foreach ( $info[7003]["code"] as $item ) {


            // افزودن به جدول وضعیت های خاص ماژول
            if ( ! DB::table( "machine_status" )->where(
                [
                    "production_status_id"   => $item,
                    "machine_module_type_id" => $machine_module_type_id
                ] )->exists() ) {

                DB::table( "machine_status" )->insert(
                    [
                        "production_status_id"   => $item,
                        "machine_module_type_id" => $machine_module_type_id
                    ]
                );
            }

        }
    }


}

