        ...
        <div class="col-sm-12" id="card">
            <div class="card">
                ...
                <div class="card-block">
                    <form id="form1"
                          action="{{route("wh.out.delivery.submit_select_new_packing_type",[$product_request_form,$page])}}"
                          method="post"
                          autocomplete="off"
                                    ...
                                    @foreach($selected_packing_list as $item)
                                        @if(isset($count_select[$item->id]) || isset($amount_select[$item->id]))
                                            <tr>
                                                <td>{{$row++}}</td>
                                                <td>{{$item->getCode()}}</td>
                                                <td>{{isset($amount_select[$item->id])?$amount_select[$item->id]:"---" }}
                                                <td>
                                                    @if(isset($count_select[$item->id]))
                                                        {{$count_select[$item->id]}}
                                                        از {{$item->sub_packing_form_number}}
                                                    @endif
                                                </td>
                                                <td>{{$item->packing_type->first_packing_type?$item->packing_type->first_packing_type->fullCaption():"فاقد بسته بندی فرعی"}}</td>
                                                <td>
