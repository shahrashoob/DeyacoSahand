...
@section("scripts")
    <script>
        ...
        all_packing_form_in_warehouse = {!! json_encode($all_packing_form_in_warehouse) !!}; // بسته بندی های خوانده شده
        ...
        function add_packing(packing_form_id, allow_alarm = true) {
            ...
            if (packing_form_id in all_packing_form_data) {
                ...
                if(all_packing_form_in_warehouse.includes(packing_form_id)){
                    $("#warning_message").html(" وضعیت بسته " + ("DCPK/" + (packing_form_id + 1000)) + " تحویل شده به انیار است.")
                    sum_accepted++;
                }
                else if (packing_form_read_ids.includes(packing_form_id)) {
                    if(all_packing_form_in_warehouse.includes(packing_form_id))
                    ...
                    $("#packing_form_read_ids").html(packing_form_read_ids)
                }
                ...
                $("#error_message").html("وضعیت بسته بندی جهت ورود به انبار نامعتبر است.")
                alarm.play();
            }
        ...
        function send_to_server() {
            ...
            request = $.ajax({
                url: "{{url( "api/warehouse/input/submit_set_to_warehouse_api" )}}",
                type: "post",
                data: {
