...
@php $row=1;@endphp
                    @foreach($form->general_items as $item)
                        ...
                        @if(isset( $product_reservoirs[$item->product_id]))

                            @foreach($product_reservoirs[$item->product_id] as $product_reservoir_item)
                                @php $k= $product_reservoir_item->reservoir_id;@endphp
                                <tr>
                                    <td>{{$row++}}
                                        <input type="hidden" style="width: 60px" min="0"
                                               name="product_reservoir[{{$k}}][form_general_item_id]"
                                    ...
                                    <td>
                                        {{$product_reservoir_item->reservoir->caption}}
                                    </td>


                                    <td >
                                        <input type="number" style="width: 60px" min="0"
                                               name="product_reservoir[{{$k}}][unit_amount]"
                                               value="{{isset($product_reservoir_rows[$k]["unit_amount"])?$product_reservoir_rows[$k]["unit_amount"]:""}}"
                                               required>
                                    </td>
