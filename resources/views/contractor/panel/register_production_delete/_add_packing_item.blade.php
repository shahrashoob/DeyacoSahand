@if($contractor_allocation->production->packing_types()->count() ==1)
    @include("component.input._lable",["id"=>"packing_type_id","value"=>$contractor_allocation->production->packing_types()->first()->packing_type->caption??"","label"=>"نوع بسته بندی"])
    @include("component.input._hidden",["id"=>"packing_type_id","value"=>$contractor_allocation->production->packing_types()->first()->packing_type->id??"",])
@endif
{{--@include("component.input._hidden",["id"=>"packing_type_id","value"=>$contractor_allocation->production->packing_type->id??""])--}}
...
@include("component.input._lable",["id"=>"product_id","value"=>$contractor_allocation->product->fullCaption(),"label"=>"کالا"])
...
<div class="row">
    ...
    <table class="table table-styling center"
        ...
        <tr>
            ...
            <th style="width: 150px">{{$contractor_allocation->production->product->unit->measurement}}</th>
            <th></th>
        </tr>
        ...
        <tr>
            ...
            <td>
                @if(!isset($add_new_lot))
                    ...
                    <input type="hidden" id="add_new_lot" value="0">
                @endif
            </td>
        </tr>
        @if(isset($add_new_lot))
            <tr>
                <td colspan="4">
                    @include("contractor.panel.register_production._add_new_lot_number")
                </td>
            </tr>
        @endif
...
<div class="col-md-12" style="text-align: center">
    ...
    وزن ناخالص کل:
    <input type="text" value="" id="sum_sub_amount" value="{{$sum_sub_amount??""}}">
    <br/>
    <br/>
...
<script>
    ...
    window.location.href = ("{{route("contractor.panel.register_production.index",[$contractor_allocation])}}");
    ...
    function add_new_product(check_sub_amount) {
        ...
        if (check_sub_amount && isNaN(parseFloat(sum_sub_amount))) {
            alert("لطفا وزن کل را وارد نمایید.")
            return 0;
        }
        if (check_sub_amount && parseFloat(sum_sub_amount) <= 0) {
        ...
        request = $.ajax({
            url: "{{url("api/contractor/add_new_packing/add_new_product_to_packing_api")}}",
            type: "post",
            data: {
