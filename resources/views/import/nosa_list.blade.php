...
@section("content")
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                @include("component.smartwizard.step",["stepInfo"=>$stepInfo,"active"=>2])
                        ...
                        <div style="text-align: center" class="col-md-12">
                            ...
                            @if($error_count>0)
                                <div class="alert alert-danger">لطفا پس از رفع خطا های موجود دوباره فایل اکسل را
                                    ...
                                    <a href="{{route("import.nosa.index")}}" class="btn btn-primary">بارگذاری
                                        مجدد</a>

                            @else
                                <form action="{{route("import.nosa.update")}}" method="post">
                                    @csrf
                        ...
                        <table class="table table-styling">
                            ...
                            @foreach($list as $item)
                                <tr class="{{$item->error==""?"":'alert-danger'}}">
                                    <td>{{$row++}}</td>
                                    <td>{{$item->series."/".$item->order_code}}</td>
                                    <td>{{$item->order_date_shamsi}}</td>
                                    <td>{{$item->product_code}}</td>
                                    <td>{{$item->order_status}}</td>
                                    <td>{{$item->order_type}}</td>
                                    <td>{{$item->number_in_carton}}</td>
                                    <td>{{$item->carton}}</td>
                                    <td>{{$item->amount}}</td>
                                    <td>{{$item->unit_caption}}</td>
                                </tr>
                                @if($item->error!="")
                                    <tr>
                                        <td colspan=8>
