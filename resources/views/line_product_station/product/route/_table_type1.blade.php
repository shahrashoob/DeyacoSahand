<div class="table-responsive">
    <table class="table table-styling center">
        ...
        <tr>
            @php $has_unit_type4=false; @endphp
            <th class="">واحد مواد اولیه وابسته به بچ</th>
            @foreach($route->line_product_station as $item)
                @php $has_unit_type4=$item->material_unit_type_id_dependent_to_batch==4?true:$has_unit_type4; @endphp
        ...
        @if($has_unit_type4)
            <tr>
                ...
                <th class="">نوع بسته بندی مواد اولیه وابسته به بچ</th>
                @foreach($route->line_product_station as $item)
                    <td>{{$item->material_packing_type_dependent_to_batch->code??""}}</td>
                @endforeach
            </tr>
        @endif
        ...
        <tr>
            <th class="">آیا بعد از انجام عملیات <br/>
                نیاز به تایید کنترل کیفیت می باشد؟
            </th>
            @foreach($route->line_product_station as $item)
                <td>{{$item->is_need_for_quality_control?"بله":"خیر"}}</td>
            @endforeach
        ...
        <tr>
            <th class="">
              نیاز به ثبت نمونه شاهد
                <br/>
                (بایگانی یک نمونه از کالا در هر بار تولید)
            </th>
            @foreach($route->line_product_station as $item)
                <td>{{$item->has_control_sample?"بله":"خیر"}}</td>
            @endforeach
        </tr>


        <tr>
            <th class="">نوع پیش نیازی شروع عملیات ماشین
                <br/>
                با توجه به مسیر محصول قبلی

            </th>
            @foreach($route->line_product_station as $item)
                @if($item->priority_number ==1)
                    <td></td>
                @else
                    <td>{{$item->line_product_start_status->caption2??""}}</td>
                @endif
        ...
        <tr>
            <th class="">
                تاخیر در شروع عملیات های (STS)
                <br/>
                ( دقیقه)
            </th>
            @foreach($route->line_product_station as $item)
                @if($item->line_product_start_status_id ==3410001)
                    <td></td>
                @else
                    <td>{{$item->delay_in_the_start_minute}}</td>
                @endif
        ...
        <tr>
            <th class="">توضیحات روش تولید</th>
            @foreach($route->line_product_station as $item)
                <td>{{$item->production_method?$item->production_method->getCode():""}}</td>
            @endforeach
        </tr>
