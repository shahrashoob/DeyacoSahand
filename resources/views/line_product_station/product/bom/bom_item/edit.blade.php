        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h5> ویرایش ردیف BOM برای {{$bom->caption}} - {{$bom->product_route->caption}}-
                        ({{$bom->product->fullCaption()}}) </h5>
                </div>
                <div class="card-block">
                    <form id="form1"
                          action="{{route("line_product_station.product.bom_item.update",[$bom,$bom_item,$product_creation_process])}}"
                          method="post"
                          autocomplete="off"
...
{{--                                    </div>--}}
                                    ...
                                    @include("component.input._text",["id"=>"amount",'label'=>"مقدار","value"=>formatDecimal9($bom_item->amount)])


                                    @include("component.input._number",["id"=>"waste_prediction",'label'=>"درصد پیش بینی ضایعات","value"=>$bom_item->waste_prediction])
                                    @include("component.input._number",["id"=>"consumption_correction_factor_prediction",'label'=>"پیش بینی ضریب اصلاح مصرف","value"=>$bom_item->consumption_correction_factor_prediction])
                                    @include("component.input._number",["id"=>"consumption_percent_of_production_channel",'label'=>"ضریب مصرف کانال تولید ","value"=>$bom_item->consumption_percent_of_production_channel])


                                    @include("component.input._number",["id"=>"number",'label'=>"تعداد ","value"=>$bom_item->number])
                                    @include("component.input._number",["id"=>"percent_of_use",'label'=>"درصد استفاده","value"=>$bom_item->percent_of_use])
...
@section("scripts")
    ...
    <script>
        ...
        $("#material_id").change(function () {
            ...
            $(".warehouse_storage").css("display", display);
        ...
        function load(){
            if(product_ids_no_warehouse.includes(parseInt("{{$bom_item->material_id}}"))){
            ...
            if (parseInt("{{$bom_item->bill_of_material_dependency_type_id}}") == 2) {
                display = 'none';
            }
            else{
                display="";
