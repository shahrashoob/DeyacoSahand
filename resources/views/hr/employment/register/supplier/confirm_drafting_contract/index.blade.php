@extends('hr.employment.register.layout._layout',["title_caption"=>"تایید قرارداد تامین کننده"])

@section('content')

    <div class="col-md-12 content-class">
        <form id="form1" method="post"
              action="{{route('hr.employment.register.supplier.confirm_drafting_contract.submit',$employment->key)}}"
              enctype="multipart/form-data" novalidate="novalidate" autocomplete="false">
            @csrf

            @php $panel_name="confirm_drafting_contract" @endphp
            @include("hr.employment.register.supplier.confirm_drafting_contract._info")
