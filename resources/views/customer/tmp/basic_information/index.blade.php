...
@section("content")
    <form id="form1" style="display: inline" action="{{route("customer_group.tmp.definition_customer.update",$customer)}}" method="post"
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        <h5>
                            {{__("customer info box")}}
                        </h5>
                    </div>
                    <div class="card-block">

                        @include("customer.tmp.basic_information._edit_customer_info")
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                ...
                <div class="card">
                    <div class="card-header">
                        <h5>{{__("customer address box")}} </h5>
                    </div>
                    <div class="card-block overflow-auto">


                        <div class="row">
                            @include("customer.tmp.basic_information._address_item_input")
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $('#form1').validate({
            rules: {
                ...
                country_of_nationality_id_auto: "required",
                caption1: "required",
                caption2: "required",
                firstname1: "required",
                firstname2: "required",
        ...
        $("#customer_type_id").change(function () {
            ...
            if ($("#customer_type_id").val() == 1) {
                ...
                $("#country_of_nationality_id_auto").parent().parent().parent().css("display", "block");
            } else {
                $("#customer_type1").css("display", "none");
                $("#customer_type2").css("display", "block");
                $("#country_of_nationality_id_auto").parent().parent().parent().css("display", "none");
            }
        });
