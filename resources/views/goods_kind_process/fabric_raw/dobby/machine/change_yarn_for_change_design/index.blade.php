        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h5>تغییر نخ پود {{$machine->getCode()}} - {{$machine->caption}}</h5>
                </div>
                <div class="card-block">
                    <form id="form1"
                          action="{{route("fabric_raw.machine.change_yarn_for_change_design.submit",$machine)}}"
                          method="post"
                          autocomplete="off"
                        ...
                        @foreach($machine_input_share_band as $item)
                            @include("component.input._text",["id"=>"lot_".($item->id)."_code","label"=>"همبافت (لات) ".$item->material->caption??""])
                        @endforeach
                        <div class="col-md-12">
