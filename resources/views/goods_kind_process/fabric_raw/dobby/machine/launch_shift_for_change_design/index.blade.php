        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h5> راه اندازی شیفت جهت تغییر کالیته {{$machine->getCode()}} - {{$machine->caption}}</h5>
                </div>
                <div class="card-block">
                    <form id="form1"
                          action="{{route("fabric_raw.machine.launch_shift_for_change_design.submit",$machine)}}"
                          method="post"
                          autocomplete="off"
                        ...
                        @if($allocation->has_product_change)
                            @include("component.input._text",["id"=>"carrier_id","label"=>"شماره غلطک پارچه خام"])
                        @endif
                        <div class="col-md-12">
