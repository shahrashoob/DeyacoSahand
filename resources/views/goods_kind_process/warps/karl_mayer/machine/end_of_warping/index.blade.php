        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h5> پایان چله کشی {{$machine->getCode()}} - {{$machine->caption}}</h5>
                </div>
                <div class="card-block">
                    <form id="form1"
                          action="{{route("warps.karl_mayer.machine.end_of_warping.submit",$machine)}}"
                          method="post"
                          autocomplete="off"
                        ...
                        @else
                            @if($product->unit->weight_conversion_rate!=1)
                            ...
                            @else
{{--                                واحد اصلی وزنی است--}}
                                @include("component.input._hidden",["id"=>"amount","value"=>0])
                                @include("component.input._lable",["id"=>"gross_weight","label"=>"وزن ناخالص ","value"=>$gross_weight])
                                @include("component.input._hidden",["id"=>"gross_weight","value"=>$gross_weight])
                            @endif
                        @endif

{{--                        واحد فرعی وزنی است--}}
                        @if($product->sub_unit && $product->sub_unit->weight_conversion_rate)
                        ...
                        <div class="col-md-12">
                            ...
                            @if($sub_amount === 0 )
                                <div class="alert alert-warning">مقدار وزن ناخالص به درستی خوانش نشد، لطفا یکبار دیگر
                                    تلاش کنید.
                                </div>
                            ...
                            @else
                                <button type="submit" class="btn btn-success">ثبت پایان چله کشی</button>
