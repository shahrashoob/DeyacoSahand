                        ...
                        @foreach($reserve_allocation_list as $item)
                            <tr>
                                ...
                                @if($item->other_allocation_count)
                                    style="vertical-align: middle"  rowspan="{{$item->other_allocation_count+1}}" @endif
                                ...
                                <td>
                                    @include("line_product_station.product.unit_of_measure_type._machine_allocation",["production"=>$item->production,"units"=>$item->production->get_allocation_amount($machine->id,1,$item->allocation_id,false,1000) ])

{{--                                    {{$item->production->get_allocation_amount($machine->id)}} {{$allocation->allocation_unit_type($item->product)}}--}}
                                </td>
                                <td>
                                    {{$item->get_datetime()}}
                                </td>
                                ...
                                @for($k=1; $k<= $item->other_allocation_count; $k++)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{route("fabric.production_card.view_card",$other_machine_allocation->production)}}">{{$other_machine_allocation->production->serial}}
