...
class Algorithm1WithInventoryAndMinMaxCapacity extends Controller {
    ...
    public static function RequestForMachine( Machine $machine, Script $script, $special_goods_kind_ids, $user_id, $emergency_time , $production_type_id  ) {
        ...
        $log_data["algorithm"]          = "Algorithm1WithInventoryAndMinMaxCapacity";
        ...
        if ( $current_allocation ) {
            ...
            $amount_of_produced = GoodsKind::getAmountFromMachineLog( $current_production->product, $machine_log_in_start_production, $machine_log_last );
...
//return $warehouse_list;
        ...
        $allocation_warehouse_percent_list = [];
        ...
        foreach ( $warehouse_list as $warehouse ) {
            ...
            foreach ( $allocation_amount_remaining_minute as $allocation_id => $minute ) {
                ...
                if ( $warehouse->time_remaining_in_minute > $minute ) {
                    ...
                    $production_percent_for_request      = $warehouse->time_remaining_in_minute / $minute * 100;
                ...
                $allocation_warehouse_percent_list[ $warehouse->id ][ $allocation_id ] = $production_percent_for_request;
        ...
        $log_data["allocation_warehouse_request_list"] = $allocation_warehouse_request_list;
        $log_data["allocation_warehouse_percent_list"] = $allocation_warehouse_percent_list;
        ...
        $log_data["allocation_warehouse_request_list_request_point"] = $allocation_warehouse_request_list_request_point;
        ...
        $warehouse_need_to_target_warehouse_request_point=[];
        foreach ( $warehouse_list as $warehouse ) {
            ...
            $warehouse_need_to_target_warehouse_request_point[ $warehouse->id ] = [];
            foreach ( $allocation_warehouse_request_list[ $warehouse->id ] as $allocation_id => $production_amount_for_request ) {

                $machine_allocation = MachineAllocation::where( "allocation_id", $allocation_id )->first();

                $bom = $machine_allocation->product->get_first_bom_from_route( $machine );
                if ( ! $bom ) {
                    $log_data["message"] = " BOM برای " . ( $machine_allocation->product->caption ?? "" . ( $machine_allocation->product_id ?? "" ) ) . "یافت نشد.";
                    $log                 = AlgorithmFunction::log( $script, 0, 41003, $contour, $log_data, $user_id, $machine->id );
...
// لیست درجه هایی که می تواند تحویل بگیرد.
                        ...
                        if ( ! $bom_item ) {
                           ...
                           // Script::SendSmd($script,$current_machine_input->allocation_id??0,"در تخصیص شماره ".$current_machine_input->allocation_id." به دلیل تغییر BOM های کالا، درخواست کالا از انبار به صورت ناقص اجرا شده");
                            // یک بار اس ام اس خطا برای این خط فعال کردم، وقتی که کارت نمونه گیری میزدند و انبارک سالن به عنوان انبار تامین انتخاب شده بود به ازای
                            // هر BOM انبار، یک پیامک ارسال می شد.
                            continue;
                        }

                        $material_amount_result = CurrentMachineInput::where( "allocation_id", $allocation_id )->
                        ...
                        if ( ! isset( $warehouse_need_to_target_warehouse_request_point[ $warehouse->id ][ $current_machine_input->consume_warehouse_id ][ $material->id ] ) ) {
                            $warehouse_need_to_target_warehouse_request_point[ $warehouse->id ][ $current_machine_input->consume_warehouse_id ][ $material->id ] = 0;
                        }
                        ...
                        $warehouse_need_to_target_warehouse_request_point[ $warehouse->id ][ $current_machine_input->consume_warehouse_id ][ $material->id ] +=
                            $amount_need_for_material_id_point;
        ...
        $log_data["warehouse_need_to_request_st"]       = $warehouse_need_to_target_warehouse_request;
        $log_data["warehouse_need_to_request_point_st"] = $warehouse_need_to_target_warehouse_request_point;
        ...
        foreach ( $warehouse_list as $warehouse ) {
            ...
            foreach ( $warehouse_need_to_target_warehouse_request[ $warehouse->id ] as $consume_warehouse_id => $material_amount_required ) {

                foreach ( $material_amount_required as $material_id => $x ) {
                    $inventory = WarehouseProduct::
                    where( [ "product_id" => $material_id, "warehouse_id" => $consume_warehouse_id ] )->
                    selectRaw( "TRUNCATE(sum(input),6) - TRUNCATE(sum(output),6) as amount" )->first();
                    $inventory = $inventory->amount ?? 0;


                    $log_data["inventory"][ $consume_warehouse_id ][ $material_id ]                                              = $inventory;
                    $warehouse_need_to_target_warehouse_request[ $warehouse->id ][ $consume_warehouse_id ][ $material_id ]       -= $inventory;
                    $warehouse_need_to_target_warehouse_request_point[ $warehouse->id ][ $consume_warehouse_id ][ $material_id ] -= $inventory;
                }
            }
        }

        $log_data["warehouse_need_to_request_st-inventory"]       = $warehouse_need_to_target_warehouse_request;
        $log_data["warehouse_need_to_request_point_st-inventory"] = $warehouse_need_to_target_warehouse_request_point;
        ...
        foreach ( $warehouse_list as $warehouse ) {
            ...
            foreach ( $warehouse_need_to_target_warehouse_request_point[ $warehouse->id ] as $consume_warehouse_id => $item_request_amount_list ) {
                foreach ( $item_request_amount_list as $product_id => $item_request_amount ) {
                    if ( $item_request_amount <= 0 ) {

                        unset( $warehouse_need_to_target_warehouse_request_point[ $warehouse->id ][ $consume_warehouse_id ][ $product_id ] );

                        // اگر قرار شده که درخواست ارسال کند، به ازای همه مواردی که کم داشته درخواست می فرستد.
                        if ( isset( $warehouse_need_to_target_warehouse_request[ $warehouse->id ][ $consume_warehouse_id ][ $product_id ] ) && $warehouse_need_to_target_warehouse_request[ $warehouse->id ][ $consume_warehouse_id ][ $product_id ] <= 0 ) {
                            unset( $warehouse_need_to_target_warehouse_request[ $warehouse->id ][ $consume_warehouse_id ][ $product_id ] );
                        }
        ...
        $result                                             = [];
        $result["material_list"]                            = $warehouse_need_to_target_warehouse_request;
