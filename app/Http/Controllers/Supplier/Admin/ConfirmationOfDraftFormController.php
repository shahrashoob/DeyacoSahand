...
class ConfirmationOfDraftFormController extends Controller {
//    public static $info = [
//        "route" => "contractor.admin.confirmation_of_draft_form.",
//        "view"  => "contractor.admin.confirmation_of_draft_form.",
//
//    ];
//    var $view_path;
//    var $route_path;
...
//        $this->route_path = ConfirmationOfDraftFormController::$info["route"];
//        $this->view_path  = ConfirmationOfDraftFormController::$info["view"];
//    }


    public function confirm_exist_form( Request $request, $supplier_id, Form $form,$applicant_type_id ) {
        ...
        if ( $form->status_id != 500000515 ) { // در انتظار تایید پیش نویس مالی فرم  خروج از انبار
        ...
        where( [ "applicant_type_id" => $applicant_type_id, "applicant_id" => $supplier_id ] )->
        select( "product_request_form_form.*" )->
        first();

        if ( ! $product_request_form_form ) {
            return back()->withErrors( "این برگ خروج برای تامین کننده وجود ندارد، لطفا با پشتیبانی تماس بگیرید." );
        }
        ...
        foreach ( $product_request_form_forms as $item ) {
            event( new ProductRequestFormLogEvent( $item->product_request_form, "", $form->id, 7005010 ) );
    ...
    public function reject_exist_form( Request $request, MachineAllocation $machine_allocation, Form $form ) {
        return back()->withErrors("این ماژول در درست پیاده سازی می باشد، لطفا با پشتیبانی تماس بگیرید.");
...
//        }
    ...
    public function checkPermission() {
        if ( ! \Auth::user()->posts->first()->checkButtonPermission( "sales._confirm_draft_form" ) ) {
            return back()->withErrors( "شما اجازه دسترسی به عملیات (تایید پیش نویس برگ خروج از انبار برای فروش ) مورد نظر را ندارید" );
        }
    }
}
