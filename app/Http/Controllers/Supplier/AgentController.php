...
{
    protected $view_path = "supplier.agent.";
    protected $route_path = "supplier.agent.";


    public function index()
    {

        $first_agent= Agent::where('user_id', Auth::user()->id)->where('status_id', 4642002)->WherenotNull('supplier_id')->first();
        if(!$first_agent){
            return redirect()->back()->withErrors("امکان ثبت نماینده نمی باشد با پشتیبانی تماس بگیرید.");
        }
        $agent_list = Agent::where('supplier_id',   $first_agent->supplier_id)->get();
    ...
    public function submit(Request $request)
    {
        ...
        $first_agent= Agent::where('user_id', Auth::user()->id)->where('status_id', 4642002)->WherenotNull('supplier_id')->first();
        $agent = Agent::create([
            'supplier_id' => $first_agent->supplier_id,
