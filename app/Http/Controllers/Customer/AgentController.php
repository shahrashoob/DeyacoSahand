...
{
    protected $view_path = "customer.agent.";
    protected $route_path = "customer_group.agent.";
    ...
    public function index()
    {
         ...
         $first_agent = Agent::where('user_id', Auth::user()->id)->where('status_id', 4642002)->WherenotNull('customer_id')->first();
        if(! $first_agent){
            return redirect()->back()->withErrors("امکان ثبت نماینده نمی باشد با پشتیبانی تماس بگیرید.");
        }
        $agent_list = Agent::where('customer_id', $first_agent->customer_id)->get();
    ...
    public function submit(Request $request)
    {
        ...
        //ابتدایک درخواست همکاری ایجاد می شود
...
//سپس کاربر ان ایجاد می شود
        ...
        $first_agent = Agent::where('user_id', Auth::user()->id)->where('status_id', 4642002)->WherenotNull('customer_id')->first();
        $agent = Agent::create([
            'customer_id' => $first_agent->customer_id,
        ...
        $employment->company_id = $agent->company_id;
        $employment->save();
