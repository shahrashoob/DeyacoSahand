 ...
 * تایید وصول مطالبات
 */
class ConfirmationOfDemandsFormController extends Controller {
    public static $info = [
        "route" => "sales.confirmation_of_demands_form.",
        "view"  => "sales.confirmation_of_demands_form.",
    ...
    public function confirm_exist_form( Request $request, Order $order, Form $form ) {
        ...
        if ( $form->status_id != 500000514 ) { // در انتظار تایید وصول مطالبات برگ  خروج از انبار
        ...
        foreach ( $product_request_form_forms as $item ) {
            event( new ProductRequestFormLogEvent( $item->product_request_form, "", $form->id, 7005025 ) );
