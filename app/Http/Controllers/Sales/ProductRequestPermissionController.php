...
class ProductRequestPermissionController extends Controller
{
    ...
    public function index_customer(Request $request, $customer_id)
    {
        ...
        return $this->index($request, $order);
    }
    ...
    public function index(Request $request, Order $order)
    {
        ...
        // مقداری که الان انتخاب کرده است.
        $inventory_current_selected = session("inventory_current_selected_" . $order->customer_id);
        ...
        $result_product = self::GetProductData($order, [], $does_sales_set_permission_for_inventory_on_the_way, $method_of_calculating_active_inventory_on_sales, $product_equal_to, $order_equal_to);
        if (!$result_product["result"] && !$result_product["warning"]) {
        ...
        $list_current_delivery_amount_all = $result_product["list_current_delivery_amount_all"];
        ...
        $customer_order_list = $result_product["customer_order_list"];
        ...
        if (session('new_permission_list_by_product_' . $order->customer_id)) {
            $new_permission_list_by_product = session('new_permission_list_by_product_' . $order->customer_id);
        } else {
            $new_permission_list_by_product = [];
        }

        $selected_all = session("selected_all_" . $order->customer_id);
        ...
        return view($this->view_path . "index", compact(
            ...
            "permission_amount", "order", "inventory_list", "list_current_delivery_amount_all",
            ...
            "all_order_list", "all_product_request_remaining_list", "selected_all",
            "new_permission_list", "order_ids_count", "back_url", "customer_order_list"
            , "route", "order_equal_to", "product_equal_to", "new_permission_list_by_product", "inventory_current_selected"
        ));
    ...
    {
        /**
         * save_status ==1 => ذخیره و ادامه
         * save_status == 2 => ذخیره
         * save_status == 3 => انتخاب تیک همه ردیف های انتخاب شده
         * save_status ==4 => حذف همه تیک های انتخاب شده
         */
        $result = self::check_permission($order);
        if (!$result["result"]) {
            return back()->withErrors($result["message"]);
        ...
        $product_equal_to = $request->hidden_product_equal_to;
        $order_equal_to = $request->hidden_order_equal_to;

        session([
            "search_product_equal_to" => $product_equal_to,
        ...
        $be_lower_in_confirm_exit_form = -1; //  درصد مجاز کمتر بودن مقدار تحویل جهت تایید فرم خروج از انبار
//        return $request->all();

        $result_product = self::GetProductData($order, [], $does_sales_set_permission_for_inventory_on_the_way, $method_of_calculating_active_inventory_on_sales, null, null, true);
        ...
        $product_request_form_in_page = $request->product_request_form_in_page;

        $inventory_current_selected = session("inventory_current_selected_" . $order->customer_id);
        //if(!$inventory_current_selected) {
        $inventory_current_selected = [];
        // }
        ...
        foreach ($product_request_form_items as $product_request_form_item) {
            ...
            if ($be_lower_in_confirm_exit_form < 0) {
                $be_lower_in_confirm_exit_form = $product_request_form_item->product->goods_kind->be_lower_in_confirm_exit_form;
            }


            if (
                (
                    isset($request->product_permission[$product_request_form_item->order_list_id]) &&
                    isset($request->checkbox_permission[$product_request_form_item->order_list_id][0])
                )
                ||
                (
                    $request->save_status == 3

                )
            ) {

                if ($request->save_status != 3) {
                    $amount = $request->product_permission[$product_request_form_item->order_list_id] + 0;
                } else {
                    $amount = round($permission_amount[$product_request_form_item->order_list_id], 5);

                    // $amount = min($amount, $active_inventory_list[$product_request_form_item->product_id]);

                    // $active_inventory_list[$product_request_form_item->product_id] -= $permission_amount[$product_request_form_item->order_list_id];
                }

                if ($amount <= 0) {

//                    // مجوز جاری را کم می کنیم.
//                    if(isset($inventory_current_selected[$product_request_form_item->product_id]) && isset($new_permission_list[$product_request_form_item->order_id][$product_request_form_item->order_list_id])){
//                        $inventory_current_selected[$product_request_form_item->product_id]-=$new_permission_list[$product_request_form_item->order_id][$product_request_form_item->order_list_id];
//                    }
                ...
                //echo $product_request_form_item->order_list_id."////<br/>";
                // جهت اینکه دوبار نتواند یک مقدار را انتخاب نمایید.
                if (!isset($new_permission_list_by_product[$product_request_form_item->product_id])) {
                    $new_permission_list_by_product[$product_request_form_item->product_id] = 0;
                }
                $new_permission_list_by_product[$product_request_form_item->product_id] += $amount;
                ...
                if (!isset($product_request_form_in_page[$product_request_form_item->order_list_id])) {
                    // ردیف هایی که در این صفحه نیستند ولی انتخاب شده اند.
                    // echo $product_request_form_item->order_list_id."***<br/>";
                } else {
                    //echo $product_request_form_item->order_list_id."&&&<br/>";
//
...
// بررسی مقدار انتخاب شده با توجه به موجودی فعال کالا ها
        $inventory_list_check = json_decode(json_encode($active_inventory_list), true);;
        $inventory_list_check2 = json_decode(json_encode($active_inventory_list), true);;
        ...
        foreach ($product_request_form_items as $product_request_form_item) {
            ...
            // اگر درصد 20 باشد و سفارش 300 و موجودی آزاد 199 باشد 199/300 می شود .66 درصد وبشتر از 20 می باشد پس باید انتخاب گردد.
            $inventory_percent=$inventory_list_check[$product_request_form_item->product_id]/$new_permission_list[$product_request_form_item->order_id][$product_request_form_item->order_list_id];
            ...
            if ($request->save_status == 3
                ...
                < 0
                ||
                $inventory_percent < $be_lower_in_confirm_exit_form / 100
                )
            ) {


                // در نظر گرفتن اینکه با توجه به حداکثر درصد قابل قبول در زمان تایید تحویل، اگر با مقدار کمتر می تواند تحویل دهد، مقدار کمتر انتخاب شود.


                if($inventory_percent < $be_lower_in_confirm_exit_form / 100) {
                    ...
                    $new_permission_list[$product_request_form_item->order_id][$product_request_form_item->order_list_id]=$inventory_list_check[$product_request_form_item->product_id];
                }
            }
        ...
        foreach ($product_request_form_items as $product_request_form_item) {
            if (!isset($inventory_current_selected[$product_request_form_item->product_id])) {
                $inventory_current_selected[$product_request_form_item->product_id] = 0;
            }
            if (isset($new_permission_list[$product_request_form_item->order_id][$product_request_form_item->order_list_id])) {
        ...
        session(["inventory_current_selected_" . $order->customer_id => $inventory_current_selected]);
        // بررسی و بازگشت در صورتی که تیک همه را کلیک کرده باشند.
        if ($request->save_status == 3) {
            session(["new_permission_list_" . $order->customer_id => $new_permission_list]);
            session(["new_permission_list_by_product_" . $order->customer_id => $new_permission_list_by_product]);
            session(["selected_all_" . $order->customer_id => 3]);

            return back()->with(["warning" => "همه ردیف هایی از سفارش که  ثبت مجوز برای آنها  امکان پذیر بود شامل " . count($new_permission_list) . " ردیف انتخاب شدند."]);
        } elseif ($request->save_status == 4) {

            session(["inventory_current_selected_" . $order->customer_id => []]);
            session(["new_permission_list_" . $order->customer_id => []]);
            session(["new_permission_list_by_product_" . $order->customer_id => []]);
            session(["selected_all_" . $order->customer_id => null]);
            return back()->with(["warning" => "انتخاب همه ردیف ها حذف شند."]);
        }

        // چک کردن اینکه بیش از مقدار موجودی فعال انتخاب نشده باشد.
        foreach ($inventory_list_check as $product_id => $inventory_list_item) {
            ...
            $inventory_list_check2[$product_id] = $inventory_list_item;
        }
...
// بررسی مقدار وزن و حجم و پیشنهاد ماشین های مجاز
    ...
    public function get_current_delivery(Order $order, Product $product)
    {
...
// وضعیت هایی که در آن برگ خروج کشیده شده است.
        ...
        $list = FormItem::whereIn("form_id", $form_ids)->
        where("product_id", $product->id)->
        selectRaw("product_id, sum(amount) as amount, form_id")->
        groupBy("form_id")->
        with("form")->get();

        return view($this->view_path . "get_current_delivery", compact("order", "list", "product"));
    ...
    public function show_packing_form_inventory(OrderList $order_list, Product $product, $type)
    {
        ...
        switch ($type) {
            case "other_packing_inventory" : // سایر بسته بندی ها
                ...
                selectRaw("packing_forms.code,packing_forms.created_at,packing_forms.status_id as status_id, packing_forms.id as id ,packing_type_id, product_id, sum(final_amount) as final_amount")->
    ...
    public static function RemoveProductRequestFormItem(ProductRequestForm $product_request_form, Product $product, $user_id = null)
    {
        ...
        event(new ProductRequestFormLogEvent($product_request_form, $message, null, 7005026, $user_id));
    ...
    public static function GetProductData(Order $order, $order_list_ids = [], $does_sales_set_permission_for_inventory_on_the_way = 0, $method_of_calculating_active_inventory_on_sales = 0,
                                                ...
                                                $product_equal_to = null,
                                                $order_equal_to = null,
                                                $all_row = false,
    )
    {

        $product_request_form_items_query = OrderList::
        ...
        when($order_equal_to, function ($query) use ($order_equal_to) {
            return $query->where(function ($query) use ($order_equal_to) {
                if (strpos($order_equal_to, '/') !== false) {// ایا متن شامل ممیز است اگر بود به این صورت اگر نبود سرچ عادی که از قبل داشتیم
                    ...
                    return $query->where("orders.code", "like", "%" . $order_equal_to . "%")
        ...
        when($product_equal_to, function ($query) use ($product_equal_to) {
            ...
            where(function ($query) use ($product_equal_to) {
                return $query->where("products.code", "like", "%" . $product_equal_to . "%")
        ...
        if ($all_row) {
            $product_request_form_items = $product_request_form_items_query->
            orderBy("orders.order_datetime", "asc")->
            get();
        } else {
            $product_request_form_items = $product_request_form_items_query->
        ...
        $customer_amount_remaining = []; //  مقدار باقی مانده درخواست های خود مشتری
        foreach ($product_request_form_items as $product_request_form_item) {
            ...
            if (!isset($customer_amount_request[$product_request_form_item->product_id])) {
                ...
                $customer_amount_remaining[$product_request_form_item->product_id] = 0;
            }
            ...
            $customer_amount_remaining[$product_request_form_item->product_id] += round(max(0, $product_request_form_item->amount_request - $product_request_form_item->amount_sent), 4);
        ...
        // به دست آوردن مقدار سفارش سایر مشتریان
        $customer_order_list = Order::join("order_list", "orders.id", "=", "order_id")->
        whereIn("orders.status_id", self::$order_status_list)->
        whereIn("product_id", $product_ids)->
        where("orders.customer_id", $order->customer_id)->
        selectRaw("sum(amount) as amount, product_id")->
        ...
        foreach ($product_request_form_items as $product_request_form_item) {
            ...
            $active_inventory =
                ...
                $does_sales_set_permission_for_inventory_on_the_way ?
                    ...
                    $customer_amount_remaining[$product_request_form_item->product_id] + // مقدار  باقی مانده درخواست های خود مشتری
                    $all_product_request_remaining_list[$product_request_form_item->product_id]  // مقدار  باقی مانده درخواست سایر مشتریان
...
//            "list_current_delivery_amount_order_list" => $list_current_delivery_amount_order_list,
            ...
            "list_current_delivery_amount_all" => $list_current_delivery_amount_all,
