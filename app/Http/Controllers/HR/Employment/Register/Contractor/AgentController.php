...
{
    protected $view_path = "hr.employment.register.contractor.agent.";
    protected $route_path = "hr.employment.register.contractor.agent.";
...
// به طور موقت از این کنترلر استفاده نمی شود.این برای زمانی است که بخواهیم در همکاری با ما نمایندگان را ایجاد کنیم
    ...
    public function submit($key, Request $request)
    {
      ...
      $employment_contractor = Employment::where([
            'national_code' => $request->national_code,
            'cooperation_type_id' => 21,
            'mobile' => $request->mobile,
            'status_id' => 4640106,
        ])->first();
        if (!$employment_contractor) {
        ...
        $agent_employment = Agent::where('user_id', $employment_contractor->user_id)->exists();
        if ($agent_employment) {
        ...
        $agent = Agent::create([
            ...
            'user_id' => $employment_contractor->user_id,
