...
namespace App\Http\Controllers\HR\Employment\Register\Supplier;
...
{
    protected $view_path = "hr.employment.register.supplier.confirm_drafting_contract.";
    protected $route_path = "hr.employment.register.supplier.confirm_drafting_contract.";


    public function index($key)
    {
        $employment = Employment::where("key", $key)->
        where("status_id", 4640113)-> //در انتظار تایدد قرارد دارد پیش نویس
        first();
        if (!$employment) {
            return redirect()->route("login")->withErrors("اطلاعات درخواست همکاری نامعتبر است.");
        }
        $keys = self::CreateKeysSupplier($employment);
        if (!$keys["result"]) {
            return back()->withErrors($keys["error"]);
        }
        $contract = Contract::where('contract_type_id', 3)->where('active_status_id', 1200)->first();//قراداد تامین کننده
        //بند
        if (!$contract) {
            return redirect()->back()->withErrors("قرارداد تامین کننده گان مشخص نشده است، لطفا با پشتیبانی تماس بگیرید.");
        }
    ...
    public function submit($key, Request $request)
    {
        ...
        $employment = Employment::where("key", $key)->
        whereIn("employments.status_id", [4640113])-> //در انتظار تایدد قرارد دارد پیش نویس
        first();
        if (!$employment) {
            return redirect()->route("login")->withErrors("اطلاعات درخواست همکاری نامعتبر است.");
        }

        $contract = Contract::where('contract_type_id', 3)->first();
        //بند
        if (!$contract) {
        ...
        if (!$company_have_separate_warehousing_software) {//در صورتی که نیاز به تایید انبار داری نداشته باشدباید مرکز هزینه براییش ایجاد کنیم
        ...
        if (!$company_have_separate_financial_software && !$company_have_separate_warehousing_software) {
            ...
            $employment->save();
            event(new EmploymentLogEvent($employment, 4640018, null, null, $employment->user_id));//ثبت مرکز هزینه
    ...
    public function print($key)
    {
        ...
        $employment = Employment::where("key", $key)->
        whereIn("employments.status_id", [4640113])->
        first();
        if (!$employment) {
            return redirect()->route("login")->withErrors("اطلاعات درخواست همکاری نامعتبر است.");
        }
...
// ذخیره اطلاعات در جدول
    ...
    public static function CreateKeysSupplier($employment)
    {
