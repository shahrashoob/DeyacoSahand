...
{
    public static $info = [
        "route" => "hr.employment.admin.supplier.drafting_contract_final.",
        "enable_status" => ["112"],
        "button" => ["caption" => "تایید نهایی پیش نویس قرارداد هوشمند (تامین کننده)", "class" => "btn-primary"],
        "view_path" => "hr.employment.admin.supplier.drafting_contract_final.",
...
//ایجادا ارایه برای توکن های مرتبط در تامین کننده
        $keys =  \App\Http\Controllers\HR\Employment\Register\Supplier\ConfirmDraftingContractController::CreateKeysSupplier($employment);
        if (!$keys["result"]) {
            return back()->withErrors($keys["error"]);
        }
        $contract = Contract::where('contract_type_id', 3)->where('active_status_id', 1200)->first();
        //بند
        if (!$contract) {
    ...
    public function submit(Employment $employment, Request $request)
    {
        ...
        if($supplier_draft_contract_confirm){
            ...
            event(new EmploymentLogEvent($employment, 4640042));//ثبت پیش نویس نهایی قرارداد هوشمند تامین کنندگان

        }else{
            ConfirmDraftInformationController::CreateCostCenterSupplier($employment);//ثبت مرکز هزینه
            event(new EmploymentLogEvent($employment, 4640042));//ثبت پیش نویس  نهایی قرارداد هوشمند تامین کنندگان
        ...
        return redirect()->route($this->dashboard_path . "view", $employment)->with(["success" => "  تایید قرارداد هوشمند تامین کنندگان با موفقیت تنظیم گردید."]);
