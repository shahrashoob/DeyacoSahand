...
/*
 این کنترلر مربوط به تایید یا عدم تاییداطلاعات دوره های آموزشی می باشد
 * */
class EducationalCourseController extends Controller
{
    ...
    public function confirm(Employment $employment)
    {
        ...
        if ($employment->status_educational_course_id != 4641401) {
            return redirect()->back()->withErrors("وضعیت در خواست همکاری جهت تایید اطلاعات آموزشی نامعتبر است.");
        }
        ...
        if ($employment->worker->user_educational_courses()->count() != 0) {
            ...
            $result_ic = UserEducationalCourse::CreateEducationalCourseInIc($employment, env("IC_APIKEY"));
            if (!$result_ic["result"]) {
        ...
        $employment->status_educational_course_id = 4641402;//تایید
        $employment->save();
        ...
        return redirect()->route($this->dashboard_path . "view", $employment)->with(["success" => "تایید دوره های آموزشی با موفقیت ثبت گردید."]);
    }
    ...
    public function reject(Employment $employment, Request $request)
    {
        ...
        $employment->status_educational_course_id = 4641403;//عدم تایید
        $employment->status_id = 4640108;//در انتظار اصلاح فرم درخواست
        $employment->save();

        event(new EmploymentLogEvent($employment, 4640010, $request->message));//عدم تایید دوره اموزشی

        $step_caption = "دوره اموزشی";
        ConfirmInfoController::SendSmsRejectInfo($employment, $request->message, $step_caption);

        return redirect()->route($this->dashboard_path . "view", $employment)->with(["success" => "عدم تایید دوره های آموزشی با موفقیت ثبت گردید."]);
    }
}
