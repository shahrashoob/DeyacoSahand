...
/*
 این کنترلر مربوط به تایید یا عدم تاییداطلاعات شغلی می باشد
 * */
class JobInformationController extends Controller
{
    ...
    public function confirm(Employment $employment)
    {
        ...
        if ($employment->status_job_information_id != 4641401) {
            return redirect()->back()->withErrors("وضعیت در خواست همکاری جهت تایید اطلاعات شغلی نامعتبر است.");
        }
        ...
        if ($employment->worker->user_job_informations()->count() != 0) {
            ...
            $result_ic = UserJobInformation::CreateJobInformationInIc($employment, env("IC_APIKEY"));
            if (!$result_ic["result"]) {
        ...
        $employment->status_job_information_id = 4641402;//تایید
        $employment->save();
        ...
        return redirect()->route($this->dashboard_path . "view", $employment)->with(["success" => "تایید اطلاعات شغلی با موفقیت ثبت گردید."]);
    }
    ...
    public function reject(Employment $employment, Request $request)
    {
        ...
        $employment->status_job_information_id = 4641403;//عدم تایید
        $employment->status_id = 4640108;//در انتظار اصلاح فرم درخواست
        $employment->save();

        event(new EmploymentLogEvent($employment, 4640009, $request->message));//عدم تاییداطلاعات شغلی


        $step_caption = "اطلاعات شغلی";
        ConfirmInfoController::SendSmsRejectInfo($employment, $request->message, $step_caption);

        return redirect()->route($this->dashboard_path . "view", $employment)->with(["success" => "عدم تایید اطلاعات شغلی با موفقیت ثبت گردید."]);
    }
}
