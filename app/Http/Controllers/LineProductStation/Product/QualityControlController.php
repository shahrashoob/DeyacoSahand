...
{
    public $route_path = "line_product_station.product.quality_control.";
    public $view_path = "line_product_station.product.quality_control.";
    ...
    public function submit(Request $request, Product $product)
    {
        ...
        if ($result["result"]) {
            return redirect()->route("line_product_station.product.planing.index", $product)->with(["success" => $result["message"]]);
        } else {
    ...
    public static function GetIndex(Product $product, $view_path, $route_path, $product_creation_process)
    {
        ...
        return view($view_path . "index", compact(
                "product",
                "product_creation_process","route_path","view_path"
            )
        );
    }
