...
{
    public static $info = [
        "route" => "line_product_station.product.product_creation.shade_number.",
        "view" => "line_product_station.product.product_creation.shade_number.",
        "enable_status" => ["018"],
        "priority_number" => 1600,
        "button" => ["caption" => "تکمیل اطلاعات شید کالا", "class" => "btn-primary"],
        "button_id" => 5231019,
    ...
    function store(Request $request, Product $product, ProductCreationProcess $product_creation_process)
    {
        ...
        $result = \App\Http\Controllers\LineProductStation\Product\ShadeNumberController::PostStore($request, $product);
        if ($result["result"]) {
    ...
    public function confirm_step( ProductCreationProcess $product_creation_process)
    {
        ...
        event(new ProductCreationProcessLogEvent($product_creation_process, 5231022));
