...
class ClassificationController extends Controller
{
    //
    public static $info = [
        "route" => "line_product_station.product.product_creation.classification.",
        "view" => "line_product_station.product.product_creation.classification.",
        "enable_status" => ["007"],
        "priority_number" => 500,
        "button" => ["caption" => "تکمیل اطلاعات طبقه بندی", "class" => "btn-primary"],
        "button_id" => 5231007,
    ...
    public function index(ProductCreationProcess $product_creation_process)
    {
        ...
        return \App\Http\Controllers\LineProductStation\Product\ClassificationController::GetIndex($product_creation_process->product, $this->view_path, $this->route_path, $product_creation_process);
    }
    ...
    public function submit(Request $request,Product $product, ProductCreationProcess $product_creation_process)
    {
        ...
        $result = \App\Http\Controllers\LineProductStation\Product\ClassificationController::PostSubmit($request, $product);
        if ($result["result"]) {
            ...
            event(new ProductCreationProcessLogEvent($product_creation_process, 5231010));
