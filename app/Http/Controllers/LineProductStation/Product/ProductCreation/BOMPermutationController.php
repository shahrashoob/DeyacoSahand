...
class BOMPermutationController extends Controller
{
    //
    public static $info = [
        "route" => "line_product_station.product.product_creation.bom_permutation.",
        "view" => "line_product_station.product.product_creation.bom_permutation.",
        "enable_status" => ["011"],
        "priority_number" => 1100,
        "button" => ["caption" => "ثبت اطلاعات کالای جایگزین تولید", "class" => "btn-primary"],
        "button_id" => 5231014,
    ...
    public function submit(Request $request,Product $product, ProductCreationProcess $product_creation_process)
    {
        ...
        $result = \App\Http\Controllers\LineProductStation\Product\BOM\BOMPermutationController::PostSubmit($request, $product);
        if ($result["result"]) {
            ...
            event(new ProductCreationProcessLogEvent($product_creation_process, 5231015));
