...
{
    public static $info = [
        "route" => "line_product_station.product.product_creation.actual_cost.",
        "view" => "line_product_station.product.product_creation.actual_cost.",
        "enable_status" => ["019"],
        "priority_number" => 1800,
        "button" => ["caption" => "تکمیل اطلاعات بهای تمام شده", "class" => "btn-primary"],
        "button_id" => 5231021,
    ...
    public function index(ProductCreationProcess $product_creation_process)
    {
        ...
        return \App\Http\Controllers\LineProductStation\Product\ActualCostController::GetIndex($product_creation_process->product, $this->view_path, $this->route_path, $product_creation_process);
    }

    public function confirm_step(Request $request,Product $product, ProductCreationProcess $product_creation_process)
    {
        $result = $this->checkPermission($product_creation_process);
        if ($result != "") {
            return $result;
            ...
            event(new ProductCreationProcessLogEvent($product_creation_process, 5231023));
