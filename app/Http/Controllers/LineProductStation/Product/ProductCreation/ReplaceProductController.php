...
{
    public static $info = [
        "route" => "line_product_station.product.product_creation.replace_product.",
        "view" => "line_product_station.product.product_creation.replace_product.",
        "enable_status" => ["012"],
        "priority_number" => 1200,
        "button" => ["caption" => "ثبت اطلاعات کالای جایگزین مصرف", "class" => "btn-primary"],
        "button_id" => 5231015,
    ...
    public function submit(Request $request,Product $product, ProductCreationProcess $product_creation_process){
        ...
        $result = \App\Http\Controllers\LineProductStation\Product\ReplaceProductController::PostSubmit( $request,$product);
        if ($result["result"]) {
    ...
    public function delete(Product $product, $replace_product_id,ProductCreationProcess $product_creation_process)
    {
        $result = $this->checkPermission($product_creation_process);
        if ($result != "") {
            return $result;
        ...
        $result =  \App\Http\Controllers\LineProductStation\Product\ReplaceProductController::GetDelete($product, $replace_product_id);
        if ($result["result"]) {
            return back()->with(["success" => $result["message"]]);
        } else {
            return back()->withErrors($result["error"]);
    ...
    public function confirm_step(ProductCreationProcess $product_creation_process)
    {
        ...
        if ($result != "") {
            ...
            event(new ProductCreationProcessLogEvent($product_creation_process, 5231016));
