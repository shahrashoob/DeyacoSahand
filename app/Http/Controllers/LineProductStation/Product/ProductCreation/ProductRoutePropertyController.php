...
class ProductRoutePropertyController extends Controller
{
    //
    public static $info = [
        "route" => "line_product_station.product.product_creation.route_property.",
        "view" => "line_product_station.product.product_creation.route_property.",
        "enable_status" => ["009"],
        "priority_number" => 900,
        "button" => ["caption" => "ثبت مشخصات مسیر محصول(ها)", "class" => "btn-primary"],
        "button_id" => 5231012,
    ...
    public function submit(Request $request,Product $product, ProductCreationProcess $product_creation_process)
    {
        ...
        $result = \App\Http\Controllers\LineProductStation\Product\ProductRoutePropertyController::PostSubmit($request, $product);
        if ($result["result"]) {

            return $this->confirm_step($product_creation_process);
    ...
    public function confirm_step( ProductCreationProcess $product_creation_process){
        ...
        event(new ProductCreationProcessLogEvent($product_creation_process, 5231013));
