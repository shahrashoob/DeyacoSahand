...
use App\Http\Controllers\GoodsKindProcess\Fabric_Raw\ProductionForm\DashboardController;
...
class FabricExtractionController extends Controller {
    ...
    public function index( ProductionForm $production_form ) {
        ...
        $machine_status = MachineStatus::where( [
            "station_id"           => $production_form->machine->station->id,
        ...
        if ( $machine_status && ! $machine_status->possibility_of_extraction_production_form ) {
            return back()->withErrors( "وضعیت ماشین جهت استخراج پارچه معتبر نمی باشد." );
        }
...
// دریافت قطب ها
        ...
        } else if ( $carrier->carrier_type_id != 4 ) {
        ...
        // حامل در حال تکمیل
        $carrier->SetStatus( 5320006, $production_form->machine->fullCaption() );

        $production_form->carrier->SetStatus( 5320005 );
        ...
        event( new ProductionFormLogEvent(
            $production_form,
            700205 // استخراج پارچه
        ) );


        // ایجاد فرم تولید جدید
        $production_form_new = ProductionForm::AddNewForm(
            $production_form->allocation_id,
            $production_form->machine->id,
            $carrier->id,
            $machineLog->id
        );

        foreach ( $production_form_new->allocation->items as $item ) {
        ...
        FabricRaw:: ChangeLot( $production_form_new->allocation );
    ...
    public function checkPermission( ProductionForm $production_form ) {
        ...
        $result = DashboardController::checkPermissionConditions( $production_form, FabricExtractionController::$info );
        if ( ! $result["result"] ) {
