...
class FailureShelvingController extends Controller {
    public static $info = [
        "route"         => "warps.karl_mayer.machine.failure_shelving.",
        "enable_status" => [ "003" ],
        "button"        => [ "caption" => "عدم قفسه گذاری", "class" => "btn-danger" ],
        "view_path"     => "goods_kind_process.warps.karl_mayer.machine.failure_shelving.",
    ...
    public function submit( Request $request, Machine $machine ) {
        ...
        MachineModuleTypeProperty::SendSms( $message, $allocation, 72030011202 );

        // اجرای ماژول پایان چله گذاری گذاری
        $result = EndOfWarpingController::submitHasAnError( $request, $machine );
        if ( ! $result["result"] ) {
            return back()->withErrors( $result["error"] );
        }


        $result = EndOfWarpingController::submitConfirm( $request, $result );
