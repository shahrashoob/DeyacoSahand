...
class ConfirmationOfDraftFormController extends Controller {
    ...
    public function confirm_exist_form( Request $request, MachineAllocation $machine_allocation, Form $form ) {
        ...
        $result = $this->checkPermission( $machine_allocation->production );
        if ( $result != "" ) {
            return $result;
        }
    ...
    public function checkPermission() {
        if ( ! \Auth::user()->posts->first()->checkButtonPermission( "contractor._confirm_draft_form" ) ) {
            return back()->withErrors( "شما اجازه دسترسی به عملیات (تایید پیش نویس برگ خروج از انبار برای پیمانکاران ) مورد نظر را ندارید" );
        }
    }
